...
class ConfigManager:
    """
    A singleton class that manages configuration settings for the oneOS system.
    ...
    def __init__(self):
        ...
        if not hasattr(self, 'config'):
            # Configuration file should be at oneos/config/config.yaml
            self.config_path = os.path.join(
                os.path.dirname(__file__),  # oneos/config directory
                'config.yaml'
            )
            self.load_config()
